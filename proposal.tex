...
%Our goal is to make such successes commonplace. I intend to leverage
...
permission-level ratings in to an app-level rating. To do
this I will design several possible aggregation methods 
(such as a weighted average) and user test them on Mechanical
Turk to determine which is the most understandable and most
useful for users. This will allow me to sort apps by their privacy 
rating as well as by their functionality rating.

I will also gather ratings for approximately 1000 apps to seed 
our prototype (Sec~\ref{subsec-gather-ratings}). I will gather these
apps from the Google Play store by choosing the most popular apps
overall as well as the most popular in various categories. After
gathering ratings for these tasks from Mechanical Turk, I will 
use these ratings to populate a database for our app.
...
\item[Fall 2015]
During this term I will finish building our marketplace app. The 
app will run on Android, and all the rating and app data will be 
stored in a Postgres database running on a Ruby server. The app 
...
on Mechanical Turk, as well as in-person studies with participants
...
apps meet their users' privacy needs (Sec~\ref{subsubsec-dev-comm}). 
This will entail calling attention to the way users are perceiving 
their app's permissions, and possibly providing actionable feedback
